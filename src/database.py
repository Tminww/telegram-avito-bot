...
from schemas.schemas import User, LinkData, Notification
DATABASE_URL = settings.database_url

async def get_user_links():
    conn = await init_db()
    rows = await conn.fetch("SELECT user_id, link FROM links")
    await conn.close()
...
async def insert_link(link_data: LinkData) -> dict:
    conn = await init_db()
    await conn.execute("INSERT INTO links (user_id, link) VALUES ($1, $2)", link_data.user_id, link_data.link)
    await conn.close()
    return {"status": "Link added"}

async def delete_link(link_data: LinkData) -> dict:
    conn = await init_db()
    await conn.execute("DELETE FROM links WHERE user_id = $1 AND link = $2", link_data.user_id, link_data.link)
    await conn.close()
